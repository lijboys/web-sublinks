// Version: 15.1.0 [2026-04-05]
// Feature: 修复 CF 版 YAML 多行块解析、特殊类型保留、Auto 误判澳大利亚国旗问题。
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/tmpl') {
      ...
      if (request.method === 'POST') {
        if (!env.ADMIN_PWD) {
          return new Response('后端未配置 ADMIN_PWD', { status: 403 });
        }
        ...
        try {
          ...
          return new Response('数据错误', { status: 400 });
        }
      }
    }
    ...
    if (url.pathname === '/api/fetch' && request.method === 'POST') {
      try {
        ...
        if (!name && disp) {
          ...
          if (m) {
            name = decodeURIComponent(m[1]).replace(/\.(txt|yaml|yml|json)$/i, '');
          }
        }

        return new Response(JSON.stringify({ text, name }), {
          status: 200,
          headers: { 'Content-Type': 'application/json' }
        });
      } catch (e) {
    ...
    const addFlag = (name) => {
      ...
      const rules = [
        { regex: /(^|[^A-Z])(HK|Hong Kong|香港|深港|广港|沪港)([^A-Z]|$)/i, flag: '🇭🇰' },
        { regex: /(^|[^A-Z])(TW|Taiwan|台湾|台灣|台北|新北|广台)([^A-Z]|$)/i, flag: '🇹🇼' },
        { regex: /(^|[^A-Z])(JP|Japan|日本|东京|大阪|埼玉|广日)([^A-Z]|$)/i, flag: '🇯🇵' },
        { regex: /(^|[^A-Z])(SG|Singapore|新加坡|狮城|广新)([^A-Z]|$)/i, flag: '🇸🇬' },
        { regex: /(^|[^A-Z])(KR|Korea|韩国|首尔|春川|广韩)([^A-Z]|$)/i, flag: '🇰🇷' },
        { regex: /(^|[^A-Z])(US|America|United States|美国|洛杉矶|圣何塞|纽约|西雅图|芝加哥|波特兰|达拉斯|广美)([^A-Z]|$)/i, flag: '🇺🇸' },
        { regex: /(^|[^A-Z])(UK|Britain|英国|伦敦)([^A-Z]|$)/i, flag: '🇬🇧' },
        { regex: /(^|[^A-Z])(FR|France|法国|巴黎)([^A-Z]|$)/i, flag: '🇫🇷' },
        { regex: /(^|[^A-Z])(DE|Germany|德国|法兰克福)([^A-Z]|$)/i, flag: '🇩🇪' },
        { regex: /(^|[^A-Z])(NL|Netherlands|荷兰|阿姆斯特丹)([^A-Z]|$)/i, flag: '🇳🇱' },
        { regex: /(^|[^A-Z])(RU|Russia|俄罗斯|莫斯科)([^A-Z]|$)/i, flag: '🇷🇺' },
        { regex: /(^|[^A-Z])(IN|India|印度|孟买)([^A-Z]|$)/i, flag: '🇮🇳' },
        { regex: /(^|[^A-Z])(AU|Australia|澳大利亚|悉尼)([^A-Z]|$)/i, flag: '🇦🇺' },
        { regex: /(^|[^A-Z])(CA|Canada|加拿大|蒙特利尔)([^A-Z]|$)/i, flag: '🇨🇦' }
      ];

      for (let r of rules) {
        if (r.regex.test(name)) return `${r.flag} ${name}`;
      }
    ...
    const tryDecodeBase64 = (str) => {
      try {
        ...
        if (/^[A-Za-z0-9+/=._-]+$/.test(cleanStr) && cleanStr.length > 20 && !String(str).includes('- name:')) {
          let fixed = cleanStr.replace(/-/g, '+').replace(/_/g, '/');
          const pad = fixed.length % 4;
          if (pad) fixed += '='.repeat(4 - pad);
          const decoded = decodeURIComponent(escape(atob(fixed)));
          if (decoded.includes('://') || decoded.includes('proxies:') || decoded.includes('- name:')) {
            return decoded;
          }
        }
      } catch (e) {}
    ...
    const buildProxyLine = (obj) => {
      ...
      for (const [k, v] of Object.entries(obj)) {
        ...
        if (Array.isArray(v)) {
          const vals = v.map(x => typeof x === 'number' ? x : `"${String(x).replace(/"/g, '\\"')}"`);
          parts.push(`${k}: [${vals.join(', ')}]`);
        } else if (typeof v === 'object') {
          const sub = [];
          for (const [kk, vv] of Object.entries(v)) {
            if (Array.isArray(vv)) {
              const vals = vv.map(x => typeof x === 'number' ? x : `"${String(x).replace(/"/g, '\\"')}"`);
              sub.push(`${kk}: [${vals.join(', ')}]`);
            } else if (typeof vv === 'object' && vv !== null) {
              const mini = [];
              for (const [mk, mv] of Object.entries(vv)) {
                mini.push(`${mk}: ${typeof mv === 'number' ? mv : `"${String(mv).replace(/"/g, '\\"')}"`}`);
              }
              sub.push(`${kk}: {${mini.join(', ')}}`);
            } else {
              if (vv === 'true' || vv === 'false' || typeof vv === 'number') {
                sub.push(`${kk}: ${vv}`);
              } else {
                sub.push(`${kk}: "${String(vv).replace(/"/g, '\\"')}"`);
              }
            }
          }
    ...
    const parseYamlProxyBlock = (block, processName) => {
      ...
      if (trim.startsWith('- {')) {
        const rep = trim.replace(/(name:\s*['"]?)([^'",}]+)(['"]?)/i, (_, p1, p2, p3) => {
          return p1 + processName(String(p2).trim()) + p3;
        });
        return `  ${rep}`;
      }
      ...
      if (['direct', 'block', 'reject', 'dns', 'urltest', 'select', 'fallback', 'load-balance', 'relay', 'selector'].includes(type)) {
        ...
        for (const [k, v] of Object.entries(o)) {
          ...
          if (v === 'true' || v === 'false') {
            ...
            obj[k] = v.slice(1, -1).split(',').map(x => x.trim().replace(/^['"]|['"]$/g, '')).filter(Boolean);
          } else {
            obj[k] = v;
          }
        }
      ...
      if (type === 'trojan') {
        return buildProxyLine({
          ...
          type: 'trojan',
          password: o.password || '',
          sni: o.sni || o.servername || o.server || '',
      ...
      const fallbackObj = { name, type };
      for (const [k, v] of Object.entries(o)) {
        if (k === 'name' || k === 'type') continue;
        if (v === '' || String(v).toLowerCase() === 'null') continue;
        if (v === 'true' || v === 'false') fallbackObj[k] = v;
        else if (!isNaN(Number(v)) && v !== '') fallbackObj[k] = Number(v);
        else fallbackObj[k] = v;
      }
      return buildProxyLine(fallbackObj);
    };
    ...
    const parseLinksToArray = (linksStr, replaceRule) => {
      ...
      const blocks = splitProxyBlocks(processedStr);
      let proxies = [];
      let nameCount = {};
      ...
      const processName = (rawName) => {
        ...
        if (replaceRule && replaceRule.find) {
          try {
            n = n.replace(new RegExp(replaceRule.find, 'gi'), replaceRule.replace || '');
          } catch (e) {}
        }
        ...
        try {
          ...
          if (line.startsWith('vless://')) {
            ...
            let proxy = {
              ...
              type: 'vless',
              uuid: u.username,
              encryption: u.searchParams.get('encryption') || 'none'
            };
            ...
            if (net === 'ws') {
              ...
              proxy['ws-opts'] = {
                ...
                headers: { Host: u.searchParams.get('host') || u.searchParams.get('sni') || u.hostname }
              };
            } else if (net === 'grpc') {
              proxy.network = 'grpc';
              proxy['grpc-opts'] = {
                'grpc-service-name': u.searchParams.get('serviceName') || u.searchParams.get('service-name') || ''
              };
            ...
            const b64 = line.substring(8).replace(/-/g, '+').replace(/_/g, '/');
          ...
          proxies.push(`  # 解析失败: ${line} -> ${e.message}`);
        }
      }
    ...
    const buildConfig = async (rawLinks, tmplUrl, replaceRule) => {
      ...
      let finalConfig = tmplText.replace(/^proxies:\s*$/m, `proxies:\n${proxiesStr}`);
      if (finalConfig === tmplText) {
        finalConfig = tmplText.replace(/proxies:\s*\n/m, `proxies:\n${proxiesStr}\n`);
      }
      if (finalConfig === tmplText) {
        finalConfig = tmplText + "\nproxies:\n" + proxiesStr;
      }
    ...
    if (url.pathname.startsWith('/sub')) {
      ...
      try {
        ...
        if (cfg.burn) {
          ...
          const now = Date.now();
          const isExpired =
            (cfg.expireAt && now > cfg.expireAt) ||
            (cfg.expireDays && now > cfg.createdAt + cfg.expireDays * 86400000);

          if (isExpired) {
            await env.MY_KV.delete(shortId);
            return new Response('410: 订阅已过期', { status: 410 });
          }

          if (cfg.maxDownloads) {
            if (!cfg.accessedIPs.includes(clientIP)) {
              if (cfg.accessedIPs.length >= cfg.maxDownloads) {
                return new Response(`403: IP 上限拦截\nIP: ${clientIP}`, { status: 403 });
              }
        ...
        if (cfg.subInfo) {
          headers['Subscription-Userinfo'] = `upload=${cfg.subInfo.up}; download=${cfg.subInfo.down}; total=${cfg.subInfo.total}; expire=${cfg.subInfo.expire}`;
        }
        ...
        if (cfg.universal) {
          ...
          if (cfg.replaceRule && cfg.replaceRule.find) {
            try {
              rawData = rawData.replace(new RegExp(cfg.replaceRule.find, 'gi'), cfg.replaceRule.replace || '');
            } catch (e) {}
          }
    ...
    <body>
      ...
      <script>
        ...
        async function loadAdminData() {
          ...
          switchTab('admin');
          document.getElementById('adminList').innerHTML = '<div style="text-align:center; padding: 20px;">⏳ 云端数据读取中...</div>';
          ...
          try {
            ...
            if (subs.length === 0) {
              document.getElementById('adminList').innerHTML = '<div style="text-align:center; padding: 20px; color: var(--text-muted);">暂无云端短链数据</div>';
              return;
            }
        ...
        async function deleteSub(id) {
          ...
          try {
            const res = await fetch('/api/del_sub', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
        ...
        function handleFile(file) {
          if (!file) return;
          if (file.type.startsWith('image/') || file.type.startsWith('video/') || file.name.endsWith('.zip')) {
            return showToast('不支持的文件格式！', '❌');
          }
        ...
        let cloudTemplates = [];
        let localTemplates = JSON.parse(localStorage.getItem('__local_tmpls') || '[]');
        ...
        async function fetchCloudTmpls() {
          try {
            cloudTemplates = await (await fetch('/api/tmpl')).json();
          } catch (e) {}
          renderTmpls();
        }
          ...
          if (mode === 'local') {
            ...
            try {
              const res = await fetch('/api/tmpl', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ pwd, tmpls: newTmpls })
              });
        ...
        function parseBytes(str) {
          ...
          if (s.includes('K')) return Math.floor(val * 1024);
        ...
        async function generateLink() {
          ...
          try {
            ...
            if (!links) throw new Error('解析内容为空');

            const finalFilename = document.getElementById('filename').value.trim() || defaultName || '';
            const maxDownVal = document.getElementById('maxDown').value;
            ...
            document.getElementById('subUrl').value = domain + '/' + txt;
        ...
        async function generateLong() {
          ...
          try {
            const { text: links, defaultName } = await getFinalInputText();
            if (!links) throw new Error('内容为空');

            const encodedData = base64_encode_js(unescape(encodeURIComponent(links)));
